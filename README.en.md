...
- **Prompt-footprint A/B (v0.4.1)**. Raw request bodies were captured by forwarding the local model endpoint through a logging HTTP proxy, driving pi against `qwen-local/qwen3.8-27b` over both `--mode rpc` and `-p`, in an isolated `PI_CODING_AGENT_DIR`. Before the fix: a fresh session that never ran `/persistent` already carried 3 extra `Available tools` lines in the system prompt (+56 tokens) and 7 instead of 4 tool definitions (+667 tokens, +723 tokens per request); the set stayed after `/sleep` and after `new_session`, and asked "list the tools you can call", the model reported the persistent tools as ordinary capabilities. After the fix: the same capture is byte-identical to the plugin-free baseline (same system-prompt hash, 4 tools, 0 snippet lines) and the model answers `read, bash, edit, write`; the kickoff request still carries 7 tools and 3 snippets and the mission completes end to end (file created, `persistent_dormant`), then `/sleep` and `new_session` return to baseline. The new offline suite `test/tool-scope-sanity.ts` (15 checks) drives the real extension factory through a fake `ExtensionAPI` that mirrors pi's activation semantics: register-implies-active, `session_start` scopes out, `/persistent` arms, `/sleep` scopes out, restoring an `active`/`dormant` mission arms, restoring an `off` or cross-workspace mission stays scoped out, branch navigation falls back, and neither direction touches built-in or foreign extension tools. It reported 6 failures before the fix and 15/15 after.
- **Reach A/B (v0.4.1)**. The same compaction path in two states (RPC {type:"compact"}, `keepRecentTokens: 50`, model `qwen-local/qwen3.8-27b`). With no mission the guard stays out: no `guard:` line in stderr, and the session entry carries pi's own `details` shape (`readFiles` / `modifiedFiles`, 2150 -> ~1142 tokens, pi's default split-turn template). With a mission running, `[pi-persistent] guard: llm compaction of 5618 tokens -> ~732 summary tokens` fires and the entry records `details.guard=pi-persistent path=llm`; the guard's own auxiliary request carries zero tools, and the continuation dispatched after compaction still carries all 7 tools with the loop intact. Cross-checks: a session with no mission contains zero `persistent-state` custom entries, versus 17 in a mission session, and nothing else installed occupies the `/sleep`, `/compact` or `/persistent` command names (9080 files scanned under `~/.pi/agent`). `test/tool-scope-sanity.ts` grew to 18 checks, covering "no mission: `session_before_compact` is not answered and out-of-root writes are not screened; mission active: the screen fires".
- Test entry points. `npm test` runs typecheck plus the guard, compaction and reach-scope unit tests. Six E2E scripts: `drive-rpc.mjs`, `probe-continuity.mjs`, `probe-qwen-flows.mjs`, `drive-qwen-restore.mjs`, `probe-qwen-fork.mjs`, `manual-compact-e2e.mjs`, all defaulting to `minimax/MiniMax-M3` at `high`, overridable with `PI_E2E_MODEL` / `PI_E2E_THINKING`.
