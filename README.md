...
- **三个 LLM 工具。** `persistent_checkpoint`、`persistent_wait` 与 `persistent_dormant`，三者都必须携带提示词里给出的 `persistent_id`，任务中途替换 mission 后，旧回合滞留的工具调用会被按 id 拒收。后者带理由安静休眠，下一条真实用户消息会自动唤醒；提示词与工具描述均明确：被拦截、结果未变化、原始请求已回答一次，都**不是** dormant 的理由。
